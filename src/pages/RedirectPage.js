...
// Simula um número de membros verossímil baseado no nome do grupo
function fakeMemberCount(seed) {
  let h = 0
  for (let i = 0; i < seed.length; i++) h = (Math.imul(31, h) + seed.charCodeAt(i)) | 0
  return 312 + (Math.abs(h) % 388) // entre 312 e 699
}
...
export default function RedirectPage() {
  ...
  const [spotsLeft] = useState(() => Math.floor(Math.random() * 8) + 3) // 3–10 vagas
  ...
  // Pulsa o botão a cada 4s para chamar atenção
  useEffect(() => {
