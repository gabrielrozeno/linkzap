...
// ── Page type selector shared between form and edit modal ──────────
    ...
    <div style={{
      ...
      <div>
        ...
        <input
          ...
          value={meta.prize || ""}
          onChange={(e) => onChange({ ...meta, prize: e.target.value })}
        />
      </div>
      <div>
        ...
        <input
          ...
          value={meta.ticket_price || ""}
          onChange={(e) => onChange({ ...meta, ticket_price: e.target.value })}
        />
      </div>
    </div>
  );
...
function EditModal({ group, onSave, onClose }) {
  const [pageType, setPageType] = useState(group.page_type || "standard");
  const [meta, setMeta] = useState(group.page_meta || {});
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase
      .from("groups")
      .update({ page_type: pageType, page_meta: meta })
      .eq("id", group.id);
    setSaving(false);
...
function GroupCard({ group, onDelete, onToggle, onCopy, onEdit, copiedId }) {
  ...
  const pageTypeLabel = PAGE_TYPES.find(p => p.value === (group.page_type || "standard"))?.label || "⭐ Padrão";

  return (
    <div
      className="card"
      style={{ padding: "20px 24px", marginBottom: 12, transition: "border-color 0.2s" }}
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, gap: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{
            width: 38, height: 38, background: "var(--green-dim)", borderRadius: 10,
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 3, flexWrap: "wrap" }}>
              ...
              }}>{group.active ? "Ativo" : "Inativo"}</span>
              {/* Page type badge */}
              <span style={{
                fontSize: 10, fontWeight: 600, padding: "2px 8px", borderRadius: 20,
                background: group.page_type === "rifa" ? "rgba(255,215,0,0.1)" : "rgba(255,255,255,0.04)",
                border: `1px solid ${group.page_type === "rifa" ? "rgba(255,215,0,0.25)" : "rgba(255,255,255,0.07)"}`,
                color: group.page_type === "rifa" ? "#ffd700" : "var(--text-muted)",
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <button className="btn btn-icon" title="Editar tipo de página" onClick={() => onEdit(group)}>✏️</button>
          <button className={`btn btn-icon ${group.active ? "active" : ""}`} onClick={() => onToggle(group.id, !group.active)}>
            {group.active ? "⏸" : "▶"}
          </button>
          <button className="btn btn-icon danger" onClick={() => onDelete(group.id)}>✕</button>
        </div>
      </div>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 13, color: "var(--green)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontFamily: "monospace" }}>
          {generatedLink}
        </span>
        <button
        ...
        >{copiedId === group.id ? "✓ Copiado" : "Copiar"}</button>
      </div>

      {/* Rifa meta preview */}
      {group.page_type === "rifa" && group.page_meta && (
        <div style={{
          marginTop: 10, background: "rgba(255,215,0,0.04)",
          border: "1px solid rgba(255,215,0,0.12)", borderRadius: 8,
          padding: "8px 14px", display: "flex", gap: 16, flexWrap: "wrap"
        }}>
          {group.page_meta.prize && (
            <span style={{ fontSize: 12, color: "rgba(255,215,0,0.7)" }}>🏆 {group.page_meta.prize}</span>
          )}
          {group.page_meta.ticket_price && (
            <span style={{ fontSize: 12, color: "rgba(37,211,102,0.7)" }}>💵 {group.page_meta.ticket_price}</span>
          )}
        </div>
      )}
    </div>
...
export default function AdminPage() {
  ...
  const handleDelete = async (id) => {
    ...
    if (!error) { setGroups((g) => g.filter((x) => x.id !== id)); showToast("Grupo removido."); }
  ...
  return (
    <div className="page">
      ...
      <div className="container">
        {/* HEADER */}
        <header style={{ padding: "44px 0 36px", borderBottom: "1px solid var(--border)", marginBottom: 44 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
              <div style={{ width: 46, height: 46, background: "#25D366", borderRadius: 13, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, boxShadow: "0 0 30px rgba(37,211,102,0.3)" }}>💬</div>
              <div>
                <div style={{ fontFamily: "var(--font-display)", fontSize: 21, fontWeight: 700, letterSpacing: "-0.3px" }}>LinkZap</div>
                <div style={{ fontSize: 11, color: "var(--text-muted)", letterSpacing: "2px", textTransform: "uppercase", marginTop: 1 }}>Painel Admin</div>
              </div>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
              {[{ val: groups.length, label: "Grupos" }, { val: activeCount, label: "Ativos" }, { val: totalClicks, label: "Cliques" }].map((s) => (
                <div key={s.label} style={{ textAlign: "right" }}>
                  <div style={{ fontFamily: "var(--font-display)", fontSize: 26, fontWeight: 800, color: "var(--green)", lineHeight: 1 }}>{s.val}</div>
                  <div style={{ fontSize: 10, color: "var(--text-muted)", letterSpacing: "1.5px", textTransform: "uppercase", marginTop: 3 }}>{s.label}</div>
                </div>
              ))}
              <button className="btn btn-ghost" onClick={signOut} style={{ fontSize: 12 }}>Sair →</button>
            </div>
          </div>
          {user && <div style={{ marginTop: 10, fontSize: 12, color: "var(--text-muted)" }}>Logado como <span style={{ color: "var(--text-dim)" }}>{user.email}</span></div>}
        </header>

        {/* TABS */}
        <div style={{ display: "flex", gap: 4, marginBottom: 32, background: "rgba(255,255,255,0.03)", border: "1px solid var(--border)", borderRadius: 12, padding: 4, width: "fit-content" }}>
          {[{ key: "groups", label: "👥 Grupos" }, { key: "sets", label: "🔀 Conjuntos (Rodízio)" }].map((tab) => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)} style={{
              padding: "8px 20px", borderRadius: 8, border: "none", cursor: "pointer",
              fontFamily: "var(--font-body)", fontSize: 13, fontWeight: 500, transition: "all 0.2s",
              background: activeTab === tab.key ? "rgba(37,211,102,0.15)" : "transparent",
              color: activeTab === tab.key ? "var(--green)" : "var(--text-dim)",
            }}>{tab.label}</button>
          ))}
        </div>
        ...
        {activeTab === "groups" && (
          <>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              ...
              <button className="btn btn-ghost" onClick={fetchGroups} style={{ fontSize: 12 }}>↻ Atualizar</button>
            </div>
            ...
            {loading ? (
              ...
              <div style={{ textAlign: "center", padding: "60px 20px", border: "1px dashed rgba(255,255,255,0.08)", borderRadius: 20 }}>
                <div style={{ fontSize: 48, marginBottom: 16, opacity: 0.25 }}>📭</div>
                <p style={{ color: "var(--text-muted)", fontSize: 14 }}>Nenhum grupo cadastrado ainda.<br />Adicione o primeiro acima!</p>
              </div>
            ) : (
              groups.map((g) => (
                <GroupCard
                  ...
                  onCopy={handleCopy} onEdit={setEditingGroup}
                  copiedId={copiedId}
                />
              ))
            )}
